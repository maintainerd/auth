...
FROM node:22-alpine AS identity
WORKDIR /app
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
    CMD curl -fsS http://localhost:8080/readyz && curl -fsS http://localhost:3000/ >/dev/null && curl -fsS http://localhost:3001/ >/dev/null || exit 1
