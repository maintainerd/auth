...
type OAuthDeviceCodeRepository interface {
	BaseRepositoryMethods[model.OAuthDeviceCode]
	WithTx(tx *gorm.DB) OAuthDeviceCodeRepository
	FindByDeviceCodeHash(hash string) (*model.OAuthDeviceCode, error)
	FindByUserCode(userCode string) (*model.OAuthDeviceCode, error)
...
type oauthDeviceCodeRepository struct {
	*BaseRepository[model.OAuthDeviceCode]
}
...
func NewOAuthDeviceCodeRepository(db *gorm.DB) OAuthDeviceCodeRepository {
	return &oauthDeviceCodeRepository{
		BaseRepository: NewBaseRepository[model.OAuthDeviceCode](db, "oauth_device_code_uuid", "oauth_device_code_id"),
	}
}
...
func (r *oauthDeviceCodeRepository) FindByDeviceCodeHash(hash string) (*model.OAuthDeviceCode, error) {
...
func (r *oauthDeviceCodeRepository) FindByUserCode(userCode string) (*model.OAuthDeviceCode, error) {
	var code model.OAuthDeviceCode
	err := r.DB().
		Preload("Client").
...
func (r *oauthDeviceCodeRepository) UpdateStatus(id int64, status string, userID *int64) error {
	...
	return r.DB().Model(&model.OAuthDeviceCode{}).
