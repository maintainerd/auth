...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE users
            ADD CONSTRAINT fk_users_organization_id FOREIGN KEY (organization_id)
...
`
	if err := db.Exec(sql).Error; err != nil {
		log.Fatalf("❌ Failed to run migration 011_create_users_table: %v", err)
	}

	log.Println("✅ Migration 011_create_users_table executed")
}
