...
func TestSecuritySettingsAuditRepository_FindBySecuritySettingID(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT \* FROM "security_settings_audit" WHERE security_setting_id = \$1 ORDER BY created_at DESC`).
			WithArgs(int64(1)).
			WillReturnRows(sqlmock.NewRows([]string{"security_settings_audit_id", "security_settings_audit_uuid", "tenant_id", "security_setting_id", "change_type"}).
...
func TestSecuritySettingsAuditRepository_FindByTenantID(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		db, mock := newSecpolicyMockGormDB(t)
		mock.ExpectQuery(`SELECT \* FROM "security_settings_audit" WHERE tenant_id = \$1 ORDER BY created_at DESC`).
			WithArgs(int64(1)).
			WillReturnRows(sqlmock.NewRows([]string{"security_settings_audit_id", "security_settings_audit_uuid", "tenant_id", "security_setting_id", "change_type"}).
				AddRow(1, testResourceUUID, 1, 1, "update_mfa_config"))

		result, err := NewSecuritySettingsAuditRepository(db).FindByTenantID(1)
		require.NoError(t, err)
		require.Len(t, result, 1)
		assert.Equal(t, "update_mfa_config", result[0].ChangeType)
		assert.NoError(t, mock.ExpectationsWereMet())
	...
	t.Run("repo error", func(t *testing.T) {
		db, mock := newSecpolicyMockGormDB(t)
		mock.ExpectQuery(`SELECT \* FROM "security_settings_audit" WHERE tenant_id = \$1`).
			WithArgs(int64(1)).
			WillReturnError(assert.AnError)
...
func TestSecuritySettingsAuditRepository_FindPaginated(t *testing.T) {
	t.Run("with TenantID filter", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT count\(\*\) FROM "security_settings_audit" WHERE tenant_id = \$1`).
			WithArgs(tenantID).
			WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))
		mock.ExpectQuery(`SELECT \* FROM "security_settings_audit" WHERE tenant_id = \$1 ORDER BY created_at DESC LIMIT \$2`).
			WithArgs(tenantID, 10).
			WillReturnRows(sqlmock.NewRows([]string{"security_settings_audit_id", "security_settings_audit_uuid", "tenant_id", "security_setting_id", "change_type"}).
				AddRow(1, testResourceUUID, tenantID, 1, "update_mfa_config"))
	...
	t.Run("with all filters", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT count\(\*\) FROM "security_settings_audit" WHERE tenant_id = \$1 AND security_setting_id = \$2 AND change_type = \$3 AND created_by = \$4`).
			WithArgs(tenantID, securitySettingID, changeType, createdBy).
			WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))
		mock.ExpectQuery(`SELECT \* FROM "security_settings_audit" WHERE tenant_id = \$1 AND security_setting_id = \$2 AND change_type = \$3 AND created_by = \$4 ORDER BY created_at DESC LIMIT \$5`).
			WithArgs(tenantID, securitySettingID, changeType, createdBy, 10).
			WillReturnRows(sqlmock.NewRows([]string{"security_settings_audit_id", "security_settings_audit_uuid", "tenant_id", "security_setting_id", "change_type"}).
