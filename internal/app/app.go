...
func NewApp(db *gorm.DB, redisClient *redis.Client) *App {
	...
	tenantUserService := service.NewTenantUserService(db, tenantUserRepo, userRepo, tenantRepo)
	...
	setupService := service.NewSetupService(db, userRepo, tenantRepo, tenantUserRepo, authClientRepo, idpRepo, roleRepo, userRoleRepo, userTokenRepo, userIdentityRepo, profileRepo)
	...
	tenantRestHandler := resthandler.NewTenantHandler(tenantService, tenantUserService)
