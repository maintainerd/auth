...
func buildInternalRouter(h *handlers, application *Application) http.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		user.UserRoute(api, h.user, h.profile, h.userTrustedDevice, userProvider, application.Cache, tenantRateLimit)
		user.UserConsentRoute(api, h.userConsent, userProvider, application.Cache, tenantRateLimit)
