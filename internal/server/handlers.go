...
type handlers struct {
	...
	registrationContext *authn.RegistrationContextHandler
...
func initHandlers(application *Application) *handlers {
	h := &handlers{
		...
		registrationContext: authn.NewRegistrationContextHandler(application.RegistrationContextService),
		...
		oauthToken:          oauth.NewOAuthTokenHandler(application.OAuthTokenService, nil, application.Cache),
	...
	// accepted as a plain Bearer token, so the binding protected nothing.
	middleware.ConfigureDPoPBinding(
		func(ctx context.Context, proof, method, requestURL, accessToken, cnfJKT string) error {
			_, err := dpop.ValidateResourceRequest(ctx, proof, method, requestURL, accessToken, cnfJKT, application.Cache)
			return err
		},
		// htu is compared against the public URL of the endpoint, not the internal
		// one the request happened to arrive on.
		func(r *http.Request) string { return config.AppPublicHostname + r.URL.Path },
	)
