...
func TestTenantRetentionRunner(t *testing.T) {
	...
	t.Run("runTenantRetention deletes expired soft-deleted tenants", func(t *testing.T) {
		db, mock := newMockGormDB(t)
		mock.ExpectBegin()
		mock.ExpectExec(`set_config\('maintainerd.allow_auth_event_delete'`).WillReturnResult(sqlmock.NewResult(0, 0))
		mock.ExpectExec(`set_config\('maintainerd.allow_management_audit_log_delete'`).WillReturnResult(sqlmock.NewResult(0, 0))
