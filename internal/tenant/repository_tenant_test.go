...
func TestTenantRepository_FindPaginated(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT count\(\*\) FROM "tenants" WHERE.*name.*display_name.*description.*status IN.*is_system`).
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))
	mock.ExpectQuery(`SELECT \* FROM "tenants" WHERE.*name.*display_name.*description.*status IN.*is_system.*ORDER BY name ASC LIMIT`).
		WillReturnRows(sqlmock.NewRows([]string{"tenant_id", "tenant_uuid", "name", "display_name", "description", "status", "is_system", "created_at", "updated_at"}).
			AddRow(1, uuid.New(), "acme", "Acme", "desc", "active", false, now, now))
