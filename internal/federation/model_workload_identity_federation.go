...
type WorkloadIdentityFederation struct {
	...
	IssuerURL                      string         `gorm:"column:issuer_url;type:varchar(512);not null" json:"issuer_url"`
	...
	// Pointer because the column has a DB default. GORM substitutes the default for
	// a zero value on INSERT, so a non-pointer bool could never persist an explicit
	// false — a federation an operator staged as disabled would go live immediately,
	// and (via a struct-based Updates, which skips zero values) an existing one could
	// never be disabled at all. For a keyless-auth trust rule that switch IS the
	// security control. nil means "use the DB default" (TRUE).
