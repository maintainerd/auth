...
func TestRegistrationFlowRepository_FindPaginated(t *testing.T) {
	...
	t.Run("success with identifier filter", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT \* FROM "registration_flows"`).
			WillReturnRows(sqlmock.NewRows([]string{"registration_flow_id", "registration_flow_uuid", "tenant_id", "name", "identifier", "client_id", "created_at", "updated_at"}).
...
func TestRegistrationFlowRepository_FindByIdentifierAndClientID(t *testing.T) {
	now := time.Now()

	t.Run("success", func(t *testing.T) {
		gdb, mock := newMockGormDBRegex(t)
		mock.ExpectQuery(`SELECT \* FROM "registration_flows" WHERE .*identifier = \$1 AND client_id = \$2`).
			WithArgs("my-flow", int64(1), 1).
			WillReturnRows(sqlmock.NewRows([]string{"registration_flow_id", "registration_flow_uuid", "tenant_id", "identifier", "client_id", "name", "created_at", "updated_at"}).
	...
	t.Run("not found returns nil", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT \* FROM "registration_flows" WHERE .*identifier = \$1 AND client_id = \$2`).
			...
			WillReturnRows(sqlmock.NewRows([]string{"registration_flow_id", "registration_flow_uuid", "tenant_id", "identifier", "client_id", "name"}))
		repo := NewRegistrationFlowRepository(gdb)
...
func TestRegistrationFlowRepository_FindByName(t *testing.T) {
	now := time.Now()

	t.Run("success", func(t *testing.T) {
		gdb, mock := newMockGormDBRegex(t)
		mock.ExpectQuery(`SELECT \* FROM "registration_flows" WHERE .*name = \$1`).
