...
func TestNewSignupFlowRepository(t *testing.T) {
	gdb, _ := newMockGormDBRegex(t)
	repo := NewSignupFlowRepository(gdb)
	assert.NotNil(t, repo)
}

func TestSignupFlowRepository_WithTx(t *testing.T) {
	gdb, _ := newMockGormDBRegex(t)
	repo := NewSignupFlowRepository(gdb)
...
func TestSignupFlowRepository_FindPaginated(t *testing.T) {
	now := time.Now()

	t.Run("success with tenant filter", func(t *testing.T) {
		gdb, mock := newMockGormDBRegex(t)
		mock.ExpectQuery(`SELECT count\(\*\) FROM "signup_flows" WHERE .*tenant_id = \$1`).
			WithArgs(int64(1)).
			WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(2))
		mock.ExpectQuery(`SELECT \* FROM "signup_flows" WHERE .*tenant_id = \$1.*ORDER BY.*LIMIT`).
			WithArgs(int64(1), 10).
	...
	t.Run("success with status filter", func(t *testing.T) {
		gdb, mock := newMockGormDBRegex(t)
		mock.ExpectQuery(`SELECT count\(\*\) FROM "signup_flows" WHERE status IN`).
			WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))
		mock.ExpectQuery(`SELECT \* FROM "signup_flows" WHERE status IN.*ORDER BY.*LIMIT`).
			WillReturnRows(sqlmock.NewRows([]string{"signup_flow_id", "signup_flow_uuid", "tenant_id", "name", "client_id", "created_at", "updated_at"}).
	...
	t.Run("success with client_id filter", func(t *testing.T) {
		gdb, mock := newMockGormDBRegex(t)
		clientID := int64(1)
		mock.ExpectQuery(`SELECT count\(\*\) FROM "signup_flows" WHERE client_id`).
			WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))
		mock.ExpectQuery(`SELECT \* FROM "signup_flows" WHERE client_id.*ORDER BY.*LIMIT`).
			WillReturnRows(sqlmock.NewRows([]string{"signup_flow_id", "signup_flow_uuid", "tenant_id", "name", "client_id", "created_at", "updated_at"}).
				AddRow(1, uuid.New(), int64(1), "flow1", int64(1), now, now))
		mock.ExpectQuery(`SELECT \* FROM "clients"`).
			WillReturnRows(sqlmock.NewRows([]string{"client_id", "client_uuid", "name", "created_at", "updated_at"}).
	...
	t.Run("success with identifier filter", func(t *testing.T) {
		gdb, mock := newMockGormDBRegex(t)
		identifier := "my-flow"
		mock.ExpectQuery(`SELECT count\(\*\) FROM "signup_flows"`).
			WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))
		mock.ExpectQuery(`SELECT \* FROM "signup_flows"`).
			WillReturnRows(sqlmock.NewRows([]string{"signup_flow_id", "signup_flow_uuid", "tenant_id", "name", "identifier", "client_id", "created_at", "updated_at"}).
...
func TestSignupFlowRepository_FindByIdentifierAndClientID(t *testing.T) {
	now := time.Now()

	t.Run("success", func(t *testing.T) {
		gdb, mock := newMockGormDBRegex(t)
		mock.ExpectQuery(`SELECT \* FROM "signup_flows" WHERE .*identifier = \$1 AND client_id = \$2`).
			WithArgs("my-flow", int64(1), 1).
			WillReturnRows(sqlmock.NewRows([]string{"signup_flow_id", "signup_flow_uuid", "tenant_id", "identifier", "client_id", "name", "created_at", "updated_at"}).
	...
	t.Run("not found returns nil", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT \* FROM "signup_flows" WHERE .*identifier = \$1 AND client_id = \$2`).
			...
			WillReturnRows(sqlmock.NewRows([]string{"signup_flow_id", "signup_flow_uuid", "tenant_id", "identifier", "client_id", "name"}))
		repo := NewSignupFlowRepository(gdb)
...
func TestSignupFlowRepository_FindByUUIDAndTenantID(t *testing.T) {
	now := time.Now()
	flowUUID := uuid.New()

	t.Run("success", func(t *testing.T) {
		gdb, mock := newMockGormDBRegex(t)
		mock.ExpectQuery(`SELECT \* FROM "signup_flows" WHERE .*signup_flow_uuid = \$1 AND tenant_id = \$2`).
			WithArgs(flowUUID, int64(1), 1).
			WillReturnRows(sqlmock.NewRows([]string{"signup_flow_id", "signup_flow_uuid", "tenant_id", "name", "created_at", "updated_at"}).
				AddRow(1, flowUUID, int64(1), "flow name", now, now))
		repo := NewSignupFlowRepository(gdb)
		result, err := repo.FindByUUIDAndTenantID(flowUUID, 1)
		require.NoError(t, err)
		assert.NotNil(t, result)
		assert.Equal(t, flowUUID, result.SignupFlowUUID)
		assert.NoError(t, mock.ExpectationsWereMet())
	})
	...
	t.Run("not found returns nil", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT \* FROM "signup_flows" WHERE .*signup_flow_uuid = \$1 AND tenant_id = \$2`).
			WithArgs(flowUUID, int64(1), 1).
			WillReturnRows(sqlmock.NewRows([]string{"signup_flow_id", "signup_flow_uuid", "tenant_id", "name"}))
		repo := NewSignupFlowRepository(gdb)
		result, err := repo.FindByUUIDAndTenantID(flowUUID, 1)
		require.NoError(t, err)
		assert.Nil(t, result)
...
func TestSignupFlowRepository_FindByName(t *testing.T) {
	now := time.Now()

	t.Run("success", func(t *testing.T) {
		gdb, mock := newMockGormDBRegex(t)
		mock.ExpectQuery(`SELECT \* FROM "signup_flows" WHERE .*name = \$1`).
