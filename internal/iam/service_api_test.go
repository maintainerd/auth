...
func newAPI(id int64, name string, tenantID int64) *API {
	return &API{
		APIID:    id,
		APIUUID:  uuid.New(),
		Name:     name,
...
func newAPIService(apiRepo *mockAPIRepo, svcRepo *mockServiceRepo) APIService {
	return NewAPIService(nil, apiRepo, svcRepo)
}
...
func TestAPIService_GetByUUID(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			name: "found → success",
			setupRepo: func(r *mockAPIRepo) {
				r.findByUUIDAndTenantIDFn = func(_ uuid.UUID, _ int64) (*API, error) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			svc := newAPIService(apiRepo, &mockServiceRepo{})
...
func TestAPIService_Get(t *testing.T) {
	t.Run("success – empty result", func(t *testing.T) {
		svc := newAPIService(&mockAPIRepo{}, &mockServiceRepo{})
	...
	t.Run("repo error → propagated", func(t *testing.T) {
		apiRepo := &mockAPIRepo{
			findPaginatedFn: func(_ APIRepositoryGetFilter) (*PaginationResult[API], error) {
				return nil, errors.New("db error")
			},
		}
		svc := newAPIService(apiRepo, &mockServiceRepo{})
	...
	t.Run("success with service preloaded", func(t *testing.T) {
		...
		api.Service = &Service{ServiceUUID: uuid.New(), Name: "users-svc"}
		apiRepo := &mockAPIRepo{
			findPaginatedFn: func(_ APIRepositoryGetFilter) (*PaginationResult[API], error) {
				return &PaginationResult[API]{
					Data: []API{*api}, Total: 1, Page: 1, Limit: 10, TotalPages: 1,
				}, nil
			},
		}
		svc := newAPIService(apiRepo, &mockServiceRepo{})
...
func TestAPIService_GetServiceIDByUUID(t *testing.T) {
	...
	t.Run("service found → returns ID", func(t *testing.T) {
		...
		svc := newAPIService(&mockAPIRepo{}, svcRepo)
...
func TestAPIService_Create(t *testing.T) {
	...
	t.Run("api already exists → error", func(t *testing.T) {
		...
		mock.ExpectRollback()
		apiRepo := &mockAPIRepo{
			findByNameFn: func(_ string, _ int64) (*API, error) {
				return newAPI(1, "users-api", tenantID), nil // already exists
			},
		}
		svc := NewAPIService(db, apiRepo, &mockServiceRepo{})
		_, err := svc.Create(context.Background(), tenantID, "users-api", "", "", shared.StatusActive, false, serviceUUID)
		require.Error(t, err)
	...
	t.Run("service not found → error", func(t *testing.T) {
		...
		svc := NewAPIService(db, &mockAPIRepo{}, svcRepo)
		_, err := svc.Create(context.Background(), tenantID, "users-api", "", "", shared.StatusActive, false, serviceUUID)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "service not found")
	})
	...
	t.Run("success", func(t *testing.T) {
		...
		result, err := svc.Create(context.Background(), tenantID, "users-api", "", "", shared.StatusActive, false, serviceUUID)
		require.NoError(t, err)
		assert.NotNil(t, result)
	})
}
...
func TestAPIService_DeleteByUUID(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			name: "not found → error",
			apiRepo: &mockAPIRepo{
				findByUUIDAndTenantIDFn: func(_ uuid.UUID, _ int64) (*API, error) { return nil, nil },
			},
			expectError: true,
			errContains: "api not found",
		},
...
func TestAPIService_Update(t *testing.T) {
	...
	t.Run("api not found → error", func(t *testing.T) {
		...
		svc := NewAPIService(db, apiRepo, &mockServiceRepo{})
		_, err := svc.Update(context.Background(), apiUUID, tenantID, "n", "d", "desc", "active", serviceUUID)
		require.Error(t, err)
	...
	t.Run("invalid service UUID → error", func(t *testing.T) {
		...
		svc := NewAPIService(db, apiRepo, &mockServiceRepo{})
		_, err := svc.Update(context.Background(), apiUUID, tenantID, "n", "d", "desc", "active", "bad-uuid")
		require.Error(t, err)
	...
	t.Run("new service NOT linked to tenant → access denied", func(t *testing.T) {
		...
		svcRepo := &mockServiceRepo{
			findByUUIDFn: func(_ any, _ ...string) (*Service, error) {
				// TenantID=0 != tenantID=1 → access denied
				return &Service{ServiceID: 5}, nil
			},
		}
		svc := NewAPIService(db, apiRepo, svcRepo)
		_, err := svc.Update(context.Background(), apiUUID, tenantID, "n", "d", "desc", "active", serviceUUID)
	...
	t.Run("name conflict → error", func(t *testing.T) {
		...
		svcRepo := &mockServiceRepo{
			findByUUIDFn: func(_ any, _ ...string) (*Service, error) {
				return &Service{ServiceID: 5, TenantID: tenantID}, nil
			},
		}
		svc := NewAPIService(db, apiRepo, svcRepo)
		_, err := svc.Update(context.Background(), apiUUID, tenantID, "new-name", "d", "desc", "active", serviceUUID)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "already exists")
	})
	...
	t.Run("update error → propagated", func(t *testing.T) {
		...
		apiRepo := &mockAPIRepo{
			...
			createOrUpdateFn: func(_ *API) (*API, error) {
		...
		svc := NewAPIService(db, apiRepo, svcRepo)
		_, err := svc.Update(context.Background(), apiUUID, tenantID, "old", "d", "desc", "active", serviceUUID)
		require.Error(t, err)
	...
	t.Run("success", func(t *testing.T) {
		...
		svc := NewAPIService(db, apiRepo, svcRepo)
		result, err := svc.Update(context.Background(), apiUUID, tenantID, "old", "New Display", "desc", "active", serviceUUID)
		require.NoError(t, err)
		assert.NotNil(t, result)
...
func TestAPIService_Create_FindByNameError(t *testing.T) {
	...
	mock.ExpectRollback()
	apiRepo := &mockAPIRepo{
		findByNameFn: func(_ string, _ int64) (*API, error) { return nil, errors.New("name lookup err") },
	}
	svc := NewAPIService(db, apiRepo, &mockServiceRepo{})
