...
func TestPolicyDocument_Validate(t *testing.T) {
	t.Run("valid", func(t *testing.T) {
		d := PolicyDocument{
			Version:   "v1",
			Statement: []PolicyStatement{{Effect: shared.PolicyEffectAllow, Action: []string{"user:*"}, Resource: []string{"auth:*"}}},
		}
		assert.NoError(t, d.Validate())
	})

	t.Run("missing version", func(t *testing.T) {
		d := PolicyDocument{Statement: []PolicyStatement{{Effect: shared.PolicyEffectAllow, Action: []string{"x"}, Resource: []string{"y"}}}}
...
func TestPolicyStatement_Validate(t *testing.T) {
	t.Run("valid allow", func(t *testing.T) {
		s := PolicyStatement{Effect: shared.PolicyEffectAllow, Action: []string{"user:*"}, Resource: []string{"auth:*"}}
		assert.NoError(t, s.Validate())
	})

	t.Run("valid deny", func(t *testing.T) {
		s := PolicyStatement{Effect: shared.PolicyEffectDeny, Action: []string{"role:delete"}, Resource: []string{"auth:roles"}}
		assert.NoError(t, s.Validate())
	})
	...
	t.Run("missing action", func(t *testing.T) {
		s := PolicyStatement{Effect: shared.PolicyEffectAllow, Resource: []string{"auth:*"}}
		require.Error(t, s.Validate())
	})
...
func TestPolicyStatusUpdateDto_Validate(t *testing.T) {
	assert.NoError(t, PolicyStatusUpdateDTO{Status: shared.StatusActive}.Validate())
