...
func (r ProfileRequestDTO) Validate() error {
	return validation.ValidateStruct(&r,
		...
		validation.Field(&r.Suffix, validation.NilOrNotEmpty, validation.RuneLength(0, 50).Error("Suffix must be at most 50 characters")),
		...
		validation.Field(&r.Bio, validation.NilOrNotEmpty, validation.RuneLength(0, 1000).Error("Bio must be at most 1000 characters")),
		validation.Field(&r.Phone, validation.NilOrNotEmpty, validation.RuneLength(0, 20).Error("Phone must be at most 20 characters")),
		...
		validation.Field(&r.Address, validation.NilOrNotEmpty, validation.RuneLength(0, 500).Error("Address must be at most 500 characters")),
		validation.Field(&r.City, validation.NilOrNotEmpty, validation.RuneLength(0, 100).Error("City must be at most 100 characters")),
		validation.Field(&r.Country, validation.NilOrNotEmpty, validation.RuneLength(2, 2).Error("Country must be a 2-character ISO code (e.g., US, PH, CA)")),
