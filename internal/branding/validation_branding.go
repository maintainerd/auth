...
func (r BrandingUpdateRequestDTO) Validate() error {
	return validation.ValidateStruct(&r,
		...
		validation.Field(&r.LogoLabel,
			validation.Length(0, 255).Error("Logo label must not exceed 255 characters"),
		),
		validation.Field(&r.LogoDetail,
			validation.Length(0, 255).Error("Logo detail must not exceed 255 characters"),
		),
		validation.Field(&r.IdentityLogoLabel,
			validation.Length(0, 255).Error("Identity logo label must not exceed 255 characters"),
		),
