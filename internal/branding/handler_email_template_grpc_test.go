...
func TestEmailTemplateGRPCHandler_RPCS(t *testing.T) {
	...
	t.Run("list success", func(t *testing.T) {
		...
		if len(res.EmailTemplates) != 1 {
			t.Fatalf("expected 1, got %d", len(res.EmailTemplates))
		}
	})
...
func TestEmailTemplateProto(t *testing.T) {
	...
	t.Run("valid input", func(t *testing.T) {
		...
		if p.EmailTemplateUuid != id.String() {
			t.Errorf("expected %s, got %s", id.String(), p.EmailTemplateUuid)
		}
		if p.Name != "welcome" {
			t.Errorf("expected welcome, got %s", p.Name)
		}
	})
}
