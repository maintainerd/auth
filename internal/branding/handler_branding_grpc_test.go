...
func TestParseUUID(t *testing.T) {
	t.Run("empty string", func(t *testing.T) {
		...
		if code := status.Code(err); code != codes.InvalidArgument { t.Errorf("expected InvalidArgument, got %v", code) }
		if id != uuid.Nil { t.Errorf("expected Nil UUID, got %s", id.String()) }
	})
	...
	t.Run("valid uuid", func(t *testing.T) {
		...
		if id != valid { t.Errorf("expected %s, got %s", valid.String(), id.String()) }
	})
}

func TestOptionalStr(t *testing.T) {
	t.Run("empty returns nil", func(t *testing.T) {
		if optionalStr("") != nil { t.Error("expected nil for empty string") }
	})

	t.Run("non-empty returns pointer", func(t *testing.T) {
		v := "hello"
		p := optionalStr(v)
		if p == nil { t.Fatal("expected non-nil pointer") }
		if *p != v { t.Errorf("expected '%s', got '%s'", v, *p) }
	})
}
...
func TestGRPCPagination(t *testing.T) {
	t.Run("nil pagination returns defaults", func(t *testing.T) {
		...
		if pg.Page != 1 { t.Errorf("expected Page=1, got %d", pg.Page) }
		if pg.Limit != 20 { t.Errorf("expected Limit=20, got %d", pg.Limit) }
	})
	...
	t.Run("zero limit falls back to default", func(t *testing.T) {
		...
		if pg.Page != 3 { t.Errorf("expected Page=3, got %d", pg.Page) }
		if pg.Limit != 20 { t.Errorf("expected Limit=20, got %d", pg.Limit) }
	})

	t.Run("full pagination", func(t *testing.T) {
		pg := grpcPagination(&authv1.Pagination{Page: 2, Limit: 50, SortBy: "name", SortOrder: "asc"})
		if pg.Page != 2 { t.Errorf("expected Page=2, got %d", pg.Page) }
		if pg.Limit != 50 { t.Errorf("expected Limit=50, got %d", pg.Limit) }
		if pg.SortBy != "name" { t.Errorf("expected SortBy=name, got %s", pg.SortBy) }
		if pg.SortOrder != "asc" { t.Errorf("expected SortOrder=asc, got %s", pg.SortOrder) }
	})
}
...
func TestPageProto(t *testing.T) {
	t.Run("converts correctly", func(t *testing.T) {
		...
		if p.Total != 42 { t.Errorf("expected Total=42, got %d", p.Total) }
		if p.Page != 2 { t.Errorf("expected Page=2, got %d", p.Page) }
		if p.Limit != 10 { t.Errorf("expected Limit=10, got %d", p.Limit) }
		if p.TotalPages != 5 { t.Errorf("expected TotalPages=5, got %d", p.TotalPages) }
	})
}

func TestBrandingProto(t *testing.T) {
	t.Run("nil input returns nil", func(t *testing.T) {
		if brandingProto(nil) != nil { t.Error("expected nil for nil input") }
	})
	...
	t.Run("valid input", func(t *testing.T) {
		...
		if p.BrandingUuid != id.String() { t.Errorf("expected %s, got %s", id.String(), p.BrandingUuid) }
		if p.CompanyName != "TestCo" { t.Errorf("expected TestCo, got %s", p.CompanyName) }
	})
}
