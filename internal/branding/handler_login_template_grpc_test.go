...
func TestStructMap(t *testing.T) {
	t.Run("nil returns nil", func(t *testing.T) {
		if structMap(nil) != nil { t.Error("expected nil for nil input") }
	})
	...
	t.Run("valid struct returns map", func(t *testing.T) {
		...
		if err != nil { t.Fatal(err) }
		m := structMap(s)
		if m == nil { t.Fatal("expected non-nil map") }
		if m["key"] != "value" { t.Errorf("expected value, got %v", m["key"]) }
	})
}

func TestLoginTemplateProto(t *testing.T) {
	t.Run("nil input returns nil", func(t *testing.T) {
		if loginTemplateProto(nil) != nil { t.Error("expected nil for nil input") }
	})
	...
	t.Run("valid input without metadata", func(t *testing.T) {
		...
		if p.LoginTemplateUuid != id.String() { t.Errorf("expected %s, got %s", id.String(), p.LoginTemplateUuid) }
		if p.Name != "default-login" { t.Errorf("expected default-login, got %s", p.Name) }
		if p.Metadata != nil { t.Errorf("expected nil metadata, got %v", p.Metadata) }
	})
	...
	t.Run("valid input with metadata", func(t *testing.T) {
		...
		p := loginTemplateProto(r)
		if p == nil { t.Fatal("expected non-nil proto") }
		if p.Metadata == nil { t.Fatal("expected non-nil metadata") }
