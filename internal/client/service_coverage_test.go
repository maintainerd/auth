...
func TestClientService_RemainingBranches(t *testing.T) {
	t.Run("create returns hash error", func(t *testing.T) {
		...
		result, err := svc.Create(context.Background(), tenantID, "name", "Name", "public", "example.com", nil, shared.StatusActive, false, uuid.NewString(), nil, true, nil, nil, nil, nil, uuid.New())
