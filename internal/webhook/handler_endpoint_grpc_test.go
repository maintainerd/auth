...
func TestWebhookEndpointGRPCHandler_RPCS(t *testing.T) {
	...
	t.Run("list success", func(t *testing.T) {
		...
		if len(res.Endpoints) != 1 {
			t.Fatalf("expected 1, got %d", len(res.Endpoints))
		}
	})

	t.Run("get success", func(t *testing.T) {
		svc := &testWebhookEndpointService{
			getByUUIDFn: func(ctx context.Context, tenantID int64, id uuid.UUID) (*WebhookEndpointServiceDataResult, error) {
				return &weResult, nil
			},
		}
		h := NewWebhookEndpointGRPCHandler(resolver, svc)
	...
	t.Run("update success", func(t *testing.T) {
		svc := &testWebhookEndpointService{
			updateFn: func(ctx context.Context, tenantID int64, weUUID uuid.UUID, url string, rotateSecret bool, subscribeAll bool, maxRetries, timeoutSeconds *int, description, rStatus string) (*WebhookEndpointServiceDataResult, error) {
				return &weResult, nil
			},
		}
		h := NewWebhookEndpointGRPCHandler(resolver, svc)
	...
	t.Run("setStatus success", func(t *testing.T) {
		svc := &testWebhookEndpointService{
			updateStatusFn: func(ctx context.Context, tenantID int64, weUUID uuid.UUID, rStatus string) (*WebhookEndpointServiceDataResult, error) {
				return &weResult, nil
			},
		}
		h := NewWebhookEndpointGRPCHandler(resolver, svc)
	...
	t.Run("create service error", func(t *testing.T) {
		svc := &testWebhookEndpointService{
			createFn: func(ctx context.Context, tenantID int64, url string, subscribeAll bool, maxRetries, timeoutSeconds *int, description, rStatus string) (*WebhookEndpointServiceDataResult, error) {
				return nil, errors.New("db")
			},
		}
		h := NewWebhookEndpointGRPCHandler(resolver, svc)
	...
	t.Run("tenant resolver error", func(t *testing.T) {
		errResolver := &testWebhookTenant{getFn: func(ctx context.Context, tuuid uuid.UUID) (*TenantServiceDataResult, error) {
	...
	t.Run("webhookProto with LastTriggeredAt", func(t *testing.T) {
		...
		if v.LastTriggeredAt == nil {
			t.Error("expected non-nil LastTriggeredAt")
		}
	})
	...
	t.Run("intPtr non-nil", func(t *testing.T) {
		...
		v := intPtr(&v32)
		if v == nil || *v != 5 {
			t.Error("expected non-nil with value 5")
		}
	})
