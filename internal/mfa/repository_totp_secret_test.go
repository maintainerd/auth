...
func TestUserTOTPSecretRepository(t *testing.T) {
	...
	t.Run("FindByUserID success not found and error", func(t *testing.T) {
		...
		expectMFASelect(mock, "user_totp_secrets").WillReturnRows(userTOTPSecretRows())
		expectMFASelect(mock, "user_totp_secrets").WillReturnError(gorm.ErrRecordNotFound)
		expectMFASelect(mock, "user_totp_secrets").WillReturnError(errors.New("db error"))
		repo := NewUserTOTPSecretRepository(db)
	...
	t.Run("Upsert creates missing secret updates existing and returns lookup error", func(t *testing.T) {
		...
		expectMFASelect(mock, "user_totp_secrets").WillReturnError(gorm.ErrRecordNotFound)
		mock.ExpectBegin()
		mock.ExpectQuery(`INSERT INTO "user_totp_secrets"`).WillReturnRows(sqlmock.NewRows([]string{"totp_secret_id"}).AddRow(1))
		mock.ExpectCommit()
		expectMFASelect(mock, "user_totp_secrets").WillReturnRows(userTOTPSecretRows())
		mock.ExpectBegin()
		expectMFAUpdate(mock, "user_totp_secrets").WillReturnResult(sqlmock.NewResult(0, 1))
		mock.ExpectCommit()
		expectMFASelect(mock, "user_totp_secrets").WillReturnError(errors.New("db error"))
		repo := NewUserTOTPSecretRepository(db)
	...
	t.Run("state mutations", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			mock.ExpectBegin()
			if i == 4 {
				expectMFADelete(mock, "user_totp_secrets").WillReturnResult(sqlmock.NewResult(0, 1))
			} else {
				expectMFAUpdate(mock, "user_totp_secrets").WillReturnResult(sqlmock.NewResult(0, 1))
			}
			mock.ExpectCommit()
	...
	t.Run("MarkStepUsed false when no rows updated", func(t *testing.T) {
		db, mock := newMockGormDB(t)
		mock.ExpectBegin()
		expectMFAUpdate(mock, "user_totp_secrets").WillReturnResult(sqlmock.NewResult(0, 0))
		mock.ExpectCommit()
