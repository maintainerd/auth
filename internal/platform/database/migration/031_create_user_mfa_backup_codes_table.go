...
// CreateUserMFABackupCodesTable stores hashed one-time backup codes for account recovery.
// code_hash is TEXT (not VARCHAR) to accommodate any hash algorithm output length.
func CreateUserMFABackupCodesTable(db *gorm.DB) error {
	return db.Exec(`
...
CREATE INDEX IF NOT EXISTS idx_user_mfa_backup_codes_code_hash ON user_mfa_backup_codes(code_hash);
`).Error
}
