...
func (r *OAuthTokenExchangeRequestDTO) Validate() error {
	...
	return validation.ValidateStruct(r,
		...
		validation.Field(&r.ActorTokenType,
			validation.When(r.ActorToken != "",
				validation.Required.Error("actor_token_type is required when actor_token is present"),
			),
		),
		validation.Field(&r.ActorToken,
			validation.When(r.ActorTokenType != "",
				validation.Required.Error("actor_token is required when actor_token_type is present"),
			),
		),
