...
func TestOAuthConnectionsService_ListConnections(t *testing.T) {
	...
	t.Run("client lookup error", func(t *testing.T) {
		db, _ := newMockDB(t)
		svc := NewOAuthConnectionsService(db, &mockClientRepo{findByIdentifierFn: func(string) (*Client, error) { return nil, errors.New("db error") }}, nil, nil)
	...
	t.Run("unknown client", func(t *testing.T) {
		db, _ := newMockDB(t)
		svc := NewOAuthConnectionsService(db, &mockClientRepo{findByIdentifierFn: func(string) (*Client, error) { return nil, nil }}, nil, nil)
		_, err := svc.ListConnections(ctx, "my-client")
		require.Error(t, err)
	})
...
/* Test removed: registrationFlow method no longer exists */
