...
Validates that the scalability design (see `docs/planning/develop-before-v0.1.0.md` Section A — cleanup runners, hot-path indexes, keyset pagination, `auth_events` monthly partitioning) holds at **1M+ users**. This is a **manual pre-release gate** run against a throwaway database, not part of CI.
