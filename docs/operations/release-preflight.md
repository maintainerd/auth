...
- [ ] **J4 load run:** k6 summary + `EXPLAIN (ANALYZE)` plans from `./load-testing.md` — p95 targets met, no seq scans on hot paths at 1M+ rows.
...
- [ ] **J6 OIDC conformance:** `/.well-known/openid-configuration` matches actual capabilities; authorize/token/PKCE/refresh/revoke/introspect/userinfo/JWKS return spec-compliant responses (see `../features/oauth2-oidc.md`).
