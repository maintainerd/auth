...
- **Multiple applications.** A single tenant can register many clients (apps) across one or more identity providers, sharing the tenant's users, roles, and settings.
...
|---|---|---|---|---|
| `:8080` | Internal / Admin | VPN-only | **maintainerd-auth-console** | Administrative management of the entire auth instance: tenant config, users, roles, permissions, clients, identity providers, templates, security settings. Has its own private login endpoint for admin users. |
...
The admin frontend. Used by the operator of the auth instance to manage everything: create and configure identity providers, clients, roles, permissions, users, templates, and security settings. Equivalent to the AWS Cognito or Auth0 management dashboard.
...
On boot, it extracts the tenant identifier from the subdomain and calls `GET :8081/tenant/{identifier}/config` to validate the tenant is active and retrieve everything needed to render the login page: branding, enabled identity providers, security settings, and signup configuration.

This frontend is fully customizable per tenant through `login_templates`.
...
│
└── Users
    ├── UserIdentities  — links user to a client/provider (stores JWT sub)
...
> The tables `branding`, `tenant_settings`, `email_config`, `sms_config`, and `webhook_endpoints` are implemented with full migrations, models, repositories, services, handlers, and routes. `security_settings` is scoped to the tenant (one row per tenant).
...
The system tenant (`is_system = true`) cannot be deleted and contains the admin users who operate the auth console (port 8080), linked via `tenant_members`. It has a default built-in identity provider and a default client identifying the `maintainerd-auth-console` frontend.

---

## Identity Providers

An **identity provider (IDP)** defines *where* authentication happens for users in the tenant.
...
Each client belongs to an identity provider (and therefore the tenant) and has a type:
...
The system tenant's built-in IDP has at least one pre-seeded client that represents **maintainerd-auth-console** (the admin frontend on port 8080).
...
- An `is_default` flag to automatically assign the role to all newly registered users in the tenant.
...
A `User` record is a global entity — it is not scoped to a tenant directly. It holds the canonical identity: `email`, `username`, `phone`, `password` (nullable for external-provider users), verification flags, and account status.
...
A `UserIdentity` record is the bridge that places a user inside a specific client/provider context. One user can have multiple identities — for example, the same person authenticated via the built-in provider and via Google.
...
The **invite system** provides controlled user onboarding. An admin or authorized user sends an invite to an email address. The recipient registers using the invite token and is automatically placed into the tenant with the pre-assigned roles.
...
### Login Template (`login_templates`)

Consumed by **auth-identity** (port 8081). Defines what end-users see on the login and registration page. The tenant's login template defines the end-user login and registration UI.
...
Per-tenant HTML and plain-text templates for all transactional emails sent by the auth system: email verification, password reset, invite, welcome, and lockout notifications.

### SMS Templates (`sms_templates`)

Per-tenant SMS message templates for one-time passwords and phone verification messages.
...
**`ip_restriction_rules`** — IP allow/block rules applied instance-wide before any tenant-level logic runs.

---

### Security Settings (`security_settings`)

Per-tenant configuration. The tenant can have entirely different values. Stored as named JSONB columns — one per concern — on a single row per tenant.
...
| Item | Status | Notes |
|---|---|---|
| `security_settings` scoping | Done | Tenant-scoped (`tenant_id`). Columns: `mfa_config`, `password_config`, `session_config`, `threat_config`, `lockout_config`, `registration_config`, `token_config`. |
